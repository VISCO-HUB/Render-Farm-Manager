...
<table class="table table-hover">
	<tr>
		...
		<th width="15%" class="hidden-xs"><span>RAM</span></th>
		<th width="15%" ng-click="orderByParam('cpu')" class="hidden-xs"><span>CPU</span> <span class="glyphicon" ng-show="orderNodes == 'cpu'" ng-class="reverse ? 'glyphicon-triangle-bottom' : 'glyphicon-triangle-top'" aria-hidden="true"></span></th>
	</tr>
	...
	ng-model="checkModel[node.ip]" uib-btn-checkbox ng-class="{disabled: isReserved(node.user) || (node.cpu > 60 && node.user != userInfo.user), reserved: node.user == userInfo.user && userInfo.user.length}" ng-mouseleave="chekSwipe($event, node.ip, isReserved(node.user) || node.cpu > 60)">
		...
		<td ><a href="" uib-popover-html="getTooltip(node.ip, node.cpudata, node.ram, node.desc)" popover-trigger="'mouseenter'" popover-class="popoverTip" popover-append-to-body="true">{{node.name}}</a></td> 
		<td >
			{{node.user}}
		</td>
		<td >
		...
		<span ng-show="!node.ram" class="text-muted">Unknown</span>
		<div class="progress" style="margin-bottom: 0;" ng-show="node.ram">				  
		  <div class="progress-bar progress-bar-striped active progress-bar-info" ng-style="{'width': getFreeRam(node.ram, node.aram) + '%'}">
			<div class="progress-text">{{getRam(node.ram, node.aram)}}<div>				
		  </div>		 		
		</div>
		</td>
		<td style="min-width: 100px;">		
		<div class="progress" style="margin-bottom: 0;">		  
		  <div class="progress-bar progress-bar-striped active" ng-style="{'width': node.cpu + '%'}" ng-class="{'progress-bar-success': node.cpu > 0, 'progress-bar-warning': node.cpu > 33, 'progress-bar-danger': node.cpu > 66}">
		  <div class="progress-text">{{node.cpu}}%<div>	
		  </div>		 		
		</div>
		</td>
	</tr>
